...
class _CreateSubmissionEventFactory(factory.Factory):
    ...
    def answer(self):
        ...
        CreateSubmissionEventAnswerObjFactory._meta.exclude = ()
        if self.submission_type == CreateSubmissionType.TEXT.value:
            CreateSubmissionEventAnswerObjFactory._meta.exclude = (
                'file_keys', 'files_descriptions')
        return CreateSubmissionEventAnswerObjFactory(submission_type=self.submission_type)
...
class _CreateSubmissionFactory(_BaseOraEventFactory):
    ...
    event = factory.LazyAttribute(
        lambda o: CreateSubmissionEventObjFactory(submission_type=o.submission_type)
    )
